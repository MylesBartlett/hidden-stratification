...
class GEORGEClassification:
    ...
    def train(self, model, train_dataloader, val_dataloader, robust=False):
        ...
        self.state = load_state_dicts(
            load_path, model, self.optimizer, self.scheduler, self.logger
        )
        ...
        train_props = np.bincount(
            np.array(train_dataloader.dataset.Y_dict["true_subclass"])
        ) / len(train_dataloader.dataset)
        val_props = np.bincount(
            np.array(val_dataloader.dataset.Y_dict["true_subclass"])
    ...
    def evaluate(
        ...
        split_props = np.bincount(
            np.array(dataloader.dataset.Y_dict["true_subclass"])
    ...
    def _run_epoch(
        ...
        metric_meters = {
            k: AverageMeter() for k in ["loss", "acc", "loss_rw", "acc_rw"]
        }
        ...
        for _, (inputs, targets) in enumerate(dataloader):
            ...
            reweight_vec = (
                None if reweight is None else reweight[targets["true_subclass"]]
            )
            ...
            acc, preds = compute_accuracy(
                logits.data, loss_targets.data, return_preds=True
            )
            ...
            if self.compute_auroc:
                ...
                targets_cat, probs_cat = torch.cat(outputs["targets"]), torch.cat(
                    outputs["probs"]
                )
    ...
    def _update_metrics(
        self, metric_meters, acc, loss, losses, corrects, batch_size, reweight_vec
    ):
    ...
    def _check_dataset(self, dataset):
        ...
        assert isinstance(
            dataset, GEORGEDataset
        ), "Dataset must subclass GEORGEDataset."
    ...
    def _compute_progress_metrics(
        ...
        for label_type, labels in type_to_labels.items():
            ...
            accs, _ = self.criterion.compute_group_avg(
                corrects, labels, num_groups=num_classes
            )
