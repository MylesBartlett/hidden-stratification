...
class GEORGEHarness:
    ...
    def get_dataloaders(self, config, mode="erm", transforms=None, subclass_labels=None):
        ...
        d = {
            ...
            "waterbirds": datasets.WaterbirdsDataset,
        }
        dataset_class = d[dataset_name]
        batch_size = config["batch_size"]

        dataloaders = {}
        for split in datasets.DATA_SPLITS:
    ...
    def get_reduction_model(self, config, nn_model=None):
        ...
        models = {
            "none": reduction.NoOpReducer,
            "pca": reduction.PCAReducer,
            "umap": reduction.UMAPReducer,
            "hardness": reduction.HardnessAugmentedReducer,
        }
        if red_config["model"] != "hardness":
            ...
            base_reduction_model = reduction.UMAPReducer(
                random_state=config["seed"], n_components=red_config["components"]
            )
            reduction_model = reduction.HardnessAugmentedReducer(nn_model, base_reduction_model)
